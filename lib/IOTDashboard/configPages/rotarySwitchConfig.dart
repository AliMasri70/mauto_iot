...
class RotarySwitchConfig extends StatefulWidget {
  const RotarySwitchConfig({super.key});

  @override
  State<RotarySwitchConfig> createState() => _RotarySwitchConfigState();
}

class _RotarySwitchConfigState extends State<RotarySwitchConfig> {
// create some values
  ...
  TextEditingController widgetName = TextEditingController(text: 'Rotary');
  TextEditingController leftPos = TextEditingController(text: '30');
  TextEditingController topPos = TextEditingController(text: '150');
  TextEditingController widgetWidth = TextEditingController(text: '200');
  TextEditingController widgetHeight = TextEditingController(text: '200');
  ...
  setLedWidget() async {
    ...
    Provider.of<RotaryState>(context, listen: false).addRotary(idSlider, 0.0);
    ...
    LedModel model = LedModel(
        ...
        sliderData: Provider.of<RotaryState>(context, listen: false)
            .getRotaryValue(idSlider)
            .toString(),
        topicName: selectedTopic,
        value: modelConn,
        onState: Center(child: Consumer<RotaryState>(
          builder: (context, rotaryState, _) {
            return MyRotatedBox(
              width: double.parse(widgetWidth.text),
              height: double.parse(widgetHeight.text),
              onValueChanged: (int value) {
                rotaryState.updateRotaryValue(context, modelConn!,
                    selectedTopic, idSlider, double.parse(value.toString()));
                print('Updated value: $value');
              },
            );
          },
        )),
        offState: Center(child: Consumer<RotaryState>(
          builder: (context, RotaryState, _) {
            return MyRotatedBox(
              width: double.parse(widgetWidth.text),
              height: double.parse(widgetHeight.text),
              onValueChanged: (int value) {
                // This function will be called whenever the value is updated
                print('Updated value: $value');
                // Add your custom logic here based on the updated value
              },
            );
          },
        )));
